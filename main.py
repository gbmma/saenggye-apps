...
if __name__ == '__main__':
    ...
    # 사이드바를 사용하여 사용자로부터 다양한 정보를 입력받습니다.
    income = st.sidebar.number_input("가계 소득을 입력하세요 (원)", min_value=0, step=1000000, value=0)
    family_members = st.sidebar.slider("가족 수를 입력하세요", min_value=1, max_value=10, value=1)
    health_status = st.sidebar.selectbox("건강 상태를 선택하세요", ["건강함", "질병/장해 있음"])
    military_service = st.sidebar.selectbox("군 복무 경력이 있습니까?", ["없음", "있음"])

    # "결과 보기" 버튼을 생성하고 클릭 시 결과를 출력하는 함수를 연결합니다.
