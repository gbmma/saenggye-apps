...
def home_page():
    ...
    st.write("병역감면 자가진단 시스템에 오신 것을 환영합니다! 이 시스템을 통해 생계곤란사유를 기반으로 한 병역감면 여부를 진단할 수 있습니다. 상세한 심사는 담당자에게 문의하십시오")
    
    st.header("생계유지 곤란사유 병역감면제도란?")
    st.write("본인이 아니면 가족의 생계를 유지할 수 없는 사람에 대해 1️⃣ 가족의 부양비, 2️⃣ 재산액, 3️⃣ 월수입액이 병역감면 기준에 모두 해당되는 경우 전시근로역으로 편입되어 경제적 배려대상자의 생활 안정을 도모하기 위한 제도입니다.)
